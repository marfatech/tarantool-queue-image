...
function tube.create_space(space_name, opts)
    ...
    space:create_index('key', { type = 'tree', parts = { 2, 'str', 4, 'str' }, unique = false})
...
function method.put(self, data, opts)
    local task = self.space.index.key:select(
        {state.READY, tostring(opts.key)},
        {limit=1}
    )[1]
    ...
    if task == nil then
        ...
        task = self.space:insert{id, state.READY, data, tostring(opts.key)}
        self.on_task_change(task, 'put')
    else
        task = self.space:update(task[1], { { '=', 3, data } })
        self.on_task_change(task, nil)
    end
